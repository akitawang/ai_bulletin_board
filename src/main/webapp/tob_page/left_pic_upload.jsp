        ...
        <div id="wrapper">
            ...
            <!-- Left Sidebar End -->
					...
					<div class="form-group col-md-12">
                        	 <h4 class="header-title">上传公告图片（左）</h4>
                         </div>
					...
					<div class="file">
						<form enctype="multipart/form-data">
						        <p><input type="file" id="csv_file1" name="csv_file1" accept="image/jpeg,image/jpg,image/png"/></p>
						        <button id="import1" type="button" class="btn btn-primary">上传左图</button>
						        <a href="/ai_bulletion_board/Index" target="_blank"><button   class="btn btn-danger">刷新大屏</button> </a>
						</form>
				    </div>
				    <script type="application/javascript">
				        $("#import1").click(function () {
				            var files = $('#csv_file1').prop('files');
				            var data = new FormData();
				            data.append('csv_file1', files[0]);
				
				            $.ajax({
				                type: 'POST',
				                url: "/ai_bulletin_board/Left_pic1_upload",
				                data: data,
				                cache: false,
				                processData: false,
				                contentType: false,
					...
					<div class="form-group col-md-12">
                        	 <h4 class="header-title">上传公告图片（右）</h4>
                         </div>
					<div class="general-label">
					...
					<div class="file">
						<form enctype="multipart/form-data">
						        <p><input type="file" id="csv_file2" name="csv_file2" accept="image/jpeg,image/jpg,image/png"/></p>
						        <button id="import2" type="button" class="btn btn-primary">上传右图</button>
						        <a href="/ai_bulletion_board/Index" target="_blank"><button   class="btn btn-danger">刷新大屏</button> </a>
						</form>
				    </div>
				    <script type="application/javascript">
				        $("#import2").click(function () {
				            var files = $('#csv_file2').prop('files');
				            var data = new FormData();
				            data.append('csv_file2', files[0]);
				
				            $.ajax({
				                type: 'POST',
				                url: "/ai_bulletin_board/Left_pic2_upload",
				                data: data,
				                cache: false,
				                processData: false,
				                contentType: false,
